...
class MusicPlayerGUI:
    ...
    # Playlist display
        self.playlist_label = ttk.Label(self.root, text="RONY'S MUSIC PLAYER", background="#FFCCCC", font=("monoid", 12, "bold"))
        self.playlist_label.grid(row=0, column=0, columnspan=3, pady=5)

        self.style.configure("Treeview.Heading", background="#add8e6", font=("Helvetica", 10, "bold"))
        
        self.playlist_tree = ttk.Treeview(self.root, columns=("Song Title", "Status"), show="headings", height=5, style="Treeview.Heading")
        ...
        self.prev_button = ttk.Button(self.root, text="⏮", command=self.playlist.play_prev_song, style="Prev.TButton")
        self.prev_button.grid(row=2, column=0, pady=5)

        # Play button: Initiates playback of the currently selected song (centered)
        self.play_button = ttk.Button(self.root, text="▶", command=self.playlist.play_current_song, style="Play.TButton")
        self.play_button.grid(row=2, column=1, pady=5)

        # Next button: Skips to the next song in the playlist and plays it
        self.next_button = ttk.Button(self.root, text="⏭", command=self.playlist.play_next_song, style="Next.TButton")
        self.next_button.grid(row=2, column=2, pady=5)

        # Pause button: Pauses the currently playing song
        self.pause_button = ttk.Button(self.root, text="| |", command=self.playlist.pause_song, style="Pause.TButton")
        self.pause_button.grid(row=3, column=0, pady=5)

        # Display button: Updates and displays the current playlist in the GUI
        self.display_button = ttk.Button(self.root, text="Display Playlist", command=self.update_playlist_display, style="Display.TButton")
        self.display_button.grid(row=3, column=1, pady=5)

        # Exit button: Exits the music player application
        self.exit_button = ttk.Button(self.root, text="Exit", command=self.exit_application, style="Exit.TButton")
        ...
        self.style.configure("Prev.TButton", background="#D6F5D6")  # red
        self.style.configure("Play.TButton", background="#ADEBAD")  # Lime color for Play button
        self.style.configure("Next.TButton", background="#D6F5D6")  # red
        self.style.configure("Pause.TButton", background="#ADC2FF")  # Gold color for Pause button
        self.style.configure("Display.TButton", background="#ADC2FF")  # Deep Pink color for Display button
        self.style.configure("Exit.TButton", background="#FFFF66")  # Saddle Brown color for Exit button
